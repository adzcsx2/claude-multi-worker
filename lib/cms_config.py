"""CMS configuration for Windows/WSL backend environment"""
import json
import os
import subprocess
import sys
...
def get_backend_env() -> str | None:
    """Get BackendEnv from env var or .cms-config.json"""
    v = (os.environ.get("CMS_BACKEND_ENV") or "").strip().lower()
    if v in {"wsl", "windows"}:
        return v
    path = Path.cwd() / ".cms-config.json"
    if path.exists():
        try:
