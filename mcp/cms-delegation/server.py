#!/usr/bin/env python3
"""
MCP stdio server for CMS cross-provider delegation.
"""
...
SERVER_INFO = {"name": "cms-delegation", "version": "0.1.0"}

CACHE_DIR = Path(
    os.environ.get("CMS_DELEGATION_CACHE_DIR")
    or (Path.home() / ".cache" / "cms" / "delegation")
)
LOG_PATH = CACHE_DIR / "mcp-server.log"
CACHE_TTL_S = int(os.environ.get("CMS_DELEGATION_TTL_S") or str(60 * 60 * 24))
...
def _pend_schema() -> dict[str, Any]:
    return {
        ...
        "properties": {
            "task_id": {
                "type": "string",
                "description": "Task id returned by cms_ask_* (optional: latest).",
            },
            "session_file": {
                "type": "string",
...
for provider in ("codex", "gemini", "claude", "opencode"):
    TOOL_DEFS.append(
        {
            "name": f"cms_ask_{provider}",
            "description": f"Submit a background request to {provider} (CMS).",
            "inputSchema": _ask_schema(),
        }
    )
    TOOL_DEFS.append(
        {
            "name": f"cms_pend_{provider}",
    ...
    TOOL_DEFS.append(
        {
            "name": f"cms_ping_{provider}",
            "description": f"Check availability for {provider} in CMS.",
...
for alias, provider, kind in ALIAS_TOOLS:
    ...
    TOOL_DEFS.append(
        {
            "name": alias,
            "description": f"Alias for cms_{kind}_{provider}.",
            "inputSchema": schema,
        }
    )
...
def _handle_tool_call(name: str, args: dict[str, Any]) -> dict[str, Any]:
    ...
    if name.startswith("cms_ask_"):
        return _submit_task(provider, args)
    if name.startswith("cms_pend_"):
        return _pend_task(provider, args)
    if name.startswith("cms_ping_"):
        return _ping_provider(provider, args)
    return _tool_error(f"unknown tool: {name}")
